...
export const HeroCard: React.FC<HeroCardProps> = ({ summary, onViewMonthDetail }) => {
  ...
  return (
    <div className="bg-white rounded-[28px] p-6 cozy-card-shadow border border-[#6FCF97]/15 relative overflow-hidden">
      {/* Nền gradient hoa văn nhẹ */}
      <div className="absolute top-0 right-0 w-36 h-36 bg-[#EBF8F1] rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none opacity-60" />
      ...
      {/* Con số trọng tâm to nhất ứng dụng */}
      <div className="my-2">
        <div className="text-4xl xs:text-[42px] font-black tracking-tight text-[#3D405B]">
          {formatVND(weeklyRemaining)}
        </div>
        <p className="text-xs font-semibold text-[#6FCF97] mt-1 flex items-center gap-1.5">
          <span>Khả dụng để tiêu tuần này</span>
          <span className="text-[10px] text-[#A7A9B4]">• ({currentWeek?.week.startDate} → {currentWeek?.week.endDate})</span>
        </p>
      </div>
      ...
      <div className="mt-4 mb-2">
        ...
        <div className="flex items-center justify-between mt-1.5 text-xs text-[#7A7D8C] font-medium">
          <span>
            Đã chi: <strong className="text-[#3D405B]">{formatVND(weeklySpent)}</strong>
          </span>
          <span>
            Hạn mức: <strong className="text-[#3D405B]">{formatVND(weeklyBudget)}</strong>
          </span>
        </div>
      </div>

      {/* Thông điệp khích lệ đáng yêu */}
      <div className="my-3.5 p-3 rounded-2xl bg-[#EBF8F1]/80 border border-[#6FCF97]/20 flex items-center gap-2.5">
        <CheckCircle2 size={18} className="text-[#58B880] shrink-0" />
        <span className="text-xs font-bold text-[#3D405B]">
          {currentWeek?.status === 'over'
            ? 'Tuần này đã vượt một chút, tuần sau sẽ khởi đầu mới nha!'
            : currentWeek?.status === 'warning'
            ? 'Sắp chạm hạn mức rồi, tiêu chậm lại xíu nhé 👀'
            : 'Bạn đang chi tiêu rất hợp lý! Hãy tiếp tục nhé 🌱'}
        </span>
      </div>
